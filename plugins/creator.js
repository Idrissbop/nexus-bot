...
let name = await conn.getName(who)
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    ...
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `رقم البوت `, `لاتقم بإزعاجه`, `Nothing`, `Morocco`, `📍 https://github.com/OVMAR-DEV/Nexus-Bot`, `Just a normal bot that sometimes has an error ☺`]
  ], fkontak)
