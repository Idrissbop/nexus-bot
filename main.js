...
global.authFile = `Nexus-BotSession`;
...
let prekey = []
let directorio = readdirSync("./Nexus-BotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./Nexus-BotSession/${files}`)
})
} 
...
const directories = ['./Nexus-BotSession/', './jadibts/']
...
setInterval(async () => {
  ...
  const bio = `ثم التطوير من طرف عمر / Nexus-Bot`;
